...
def fuzzy_score(brand, seller, title):
    ...
    seller_sim = fuzz.ratio(b, seller.lower()) / 100
    ...
    if 0.6 < seller_sim < 0.99:
        score = score + 0.4
        reasons.append("seller name similar to brand (" + str(int(seller_sim * 100)) + "% match)")

    if title_sim > 0.85 and b not in title.lower():
        score = score + 0.3
        reasons.append("title close match to brand (" + str(int(title_sim * 100)) + "%)")
