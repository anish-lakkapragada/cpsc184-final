...
if st.button("Run", type="primary", disabled=not brand):
    ...
    for name in sources:
        ...
        try:
            ...
            st.write(name + ": " + str(len(items)) + " listings fetched")
    ...
    if all_items:
        ...
        st.write("Scanned " + str(report.total_scanned) + " listings, flagged " + str(len(report.flagged)))
        ...
        if not report.flagged:
            ...
            for f in report.flagged:
                ...
                with st.container(border=True):
                    st.markdown("**[" + f.listing.title + "](" + f.listing.url + ")**")
                    st.caption("Seller: " + (f.listing.seller or "—") + "  ·  Price: $" + str(f.listing.price))
